...
// Local Caelus task result types — feed the classify / slot-state /
// context-shift endpoints. Kept on top of the rebase; upstream-PR-ready as a
// separable patch if these endpoints land upstream.
...
// Upstream renamed/refactored server_prompt_checkpoint into server_prompt_data
// (main + drft instead of pos_min/pos_max/data). server_prompt below references
// the new shape, so we go with upstream's.
